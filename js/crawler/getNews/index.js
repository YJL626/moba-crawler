...
const getArticleInfo_1 = require("./getArticleInfo");
...
const getNews = async (browser) => {
    ...
    while (len) {
        console.log(len);
        if (config_1.isTest) {
            await getArticleInfo_1.getArticleInfo(browser, newsList[0]);
            break;
        }
        // 并行执行多个
        runningList.push(getArticleInfo_1.getArticleInfo(browser, newsList[--len]));
